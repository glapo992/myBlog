...
def user (username):
    ...
    posts = user.posts.order_by(Posts.timestamp.desc()).paginate(page = page, per_page = current_app.config['POST_PER_PAGE'], error_out = False) # shows some posts per page ( see pagination )
    next_url = url_for('main.user', username=user.username, page = posts.next_num) if posts.has_next else None # next_num is a Paginate() atribute
    prev_url = url_for('main.user', username=user.username, page = posts.prev_num) if posts.has_prev else None # prev_num is a Paginate() atribute
...
@bp.route('/user/<username>/edit_profile', methods=['GET', 'POST']) # this method accepts also post requests, as specified in the html from 
@login_required  # view is protected by non-logged users
...
def feed():
    ...
    next_url = url_for('main.feed', page = posts.next_num) if posts.has_next else None # next_num is a Paginate() atribute
    prev_url = url_for('main.feed', page = posts.prev_num) if posts.has_prev else None # prev_num is a Paginate() atribute
...
def explore():
    ...
    posts = Posts.query.order_by(Posts.timestamp.desc()).paginate(page = page, per_page = current_app.config['POST_PER_PAGE_EXPLORE'], error_out = False)

    # creation of url to send to the template to naviagate the pagination
    next_url = url_for('main.explore', page = posts.next_num) if posts.has_next else None # next_num is a Paginate() atribute
    prev_url = url_for('main.explore', page = posts.prev_num) if posts.has_prev else None # prev_num is a Paginate() atribute
