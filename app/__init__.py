...
try:
    db = SQLAlchemy()    # creation of the database obj
    migrate = Migrate() # the migration engine, creates a repo (folder migrations in the root of the proj) with all the db versions. 
...
#Timezone---------------------------
moment = Moment() # wrapper of moment.js, all template must include js library (just add in base.html)
#-----------------------------------
...
mail = Mail() # use the same mail-server configured for the log erorr (see below)
#-----------------------------------
...
def create_app(config_class = Config):
    ...
    app.register_blueprint(main_bp) # prefix is optional, add to all the bp routes this prefix
    #-----------------------------------
...
def get_locale():
    ...
    best_match() compares the preferred langs in the request with the supported ones on the current app istance.
...
from app import models # this goes at the bottom to avoid circular imports. here goes the modules that imports app
